...
class LabeledDataset(Dataset):
    def __init__(self, num_users: int, num_items: int, rating_mat: dict, interactions: list, neg_ratio: float):
    ...
    def negative_sampling(self):  # uniform sampling
        neg_samples = []
        all_users = np.arange(self.num_users)
        all_items = np.arange(self.num_items)
        for user in all_users:
            user_pos_items = np.array(self.rating_mat[user].keys())
            user_unseen_items = np.delete(all_items, user_pos_items)
            user_neg_items = np.random.choice(user_unseen_items, size=int(self.neg_ratio * len(user_pos_items)), replace=False)
            user_neg_samples = zip([user]*len(user_neg_items), list(user_neg_items), [0]*len(user_neg_items))
            neg_samples += user_neg_samples

        self.train_samples = self.interactions + neg_samples  # list: [(user, item, 0 or 1)]

    def __len__(self):
        return int((1 + self.neg_ratio) * len(self.interactions))

    def __getitem__(self, index):
        return self.train_samples[index][0], self.train_samples[index][1], self.train_samples[index][2]


class EvalSample(Dataset):
    """
    ...
    def __init__(self, user_count, test_sample, valid_sample, candidates, batch_size=1024):
        ...
        batch_size : int, optional
            by default 1024
        """
        super(EvalSample, self).__init__()
        ...
        for user in range(user_count):
            if user not in test_sample:
                ...
                self.candidates.append(list(candidates[user]))
